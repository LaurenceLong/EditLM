...
def main():
    ...
    for split in ["train", "validation", "test"]:
        ...
        print("处理和保存预测任务数据...")
        torch.save(
            torch.from_numpy(original_sequences).long(),
            os.path.join(args.output_dir, f"{split}_prediction_inputs.pt")
        )
        ...
        for seq_idx in tqdm(range(len(sampled_sequences)), desc=f"创建编辑任务数据"):
        ...
        # 保存元数据
        metadata = {
            "num_prediction_sequences": len(original_sequences),
