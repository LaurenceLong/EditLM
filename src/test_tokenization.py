...
def load_meta_data(data_dir):
    # 加载元数据
    metadata_path = os.path.join(data_dir, "train_metadata.pt")
...
def main():
    ...
    try:
        ...
        for i in range(min(args.num_samples, len(deletion_dataset))):
            ...
            print(
                f"输入序列（标记删除位置）: \n{colored_text[:1024].replace('[RED]', colored('', 'red', attrs=['bold'])).replace('[/RED]', '')}..." if len(
                    colored_text) > 1024 else f"输入序列（标记删除位置）: \n{colored_text.replace('[RED]', colored('', 'red', attrs=['bold'])).replace('[/RED]', '')}")
            print(f"执行删除后的序列: \n{modified_text[:1024]}..." if len(
                modified_text) > 1024 else f"执行删除后的序列: \n{modified_text}")
        ...
        for i in range(min(args.num_samples, len(insertion_dataset))):
            ...
            print(
                f"输入序列（标记插入位置）: \n{colored_text[:1024].replace('[GREEN]', colored('', 'green', attrs=['bold'])).replace('[/GREEN]', '')}..." if len(
                    colored_text) > 1024 else f"输入序列（标记插入位置）: \n{colored_text.replace('[GREEN]', colored('', 'green', attrs=['bold'])).replace('[/GREEN]', '')}")
            print(f"执行插入后的序列: \n{modified_text[:1024]}..." if len(
                modified_text) > 1024 else f"执行插入后的序列: \n{modified_text}")
